...
const PortfolioPage: React.FC = () => {
  ...
  return (
    <Container maxWidth={false} sx={{ py: 4, px: { xs: 2, sm: 3, md: 4 }, maxWidth: '1920px', width: '100%' }}>
          ...
          {orders.length === 0 && !error && (
            <Alert severity="info" sx={{ mt: 2 }}>
              <Typography variant="body2" component="div">
                <div>Upload your Order Tracker CSV from ATrad to analyze gain/loss per individual lot.</div>
                <div>Upload your Portfolio Watchlist (latest prices) CSV to analyze unrealized gain/loss.</div>
                <div>Upload your Portfolio CSV (commission data) to analyze realized gain/loss.</div>
              </Typography>
            </Alert>
          )}
        </CardContent>
        ...
        <>
          ...
          <Grid container spacing={3} sx={{ mb: 4 }}>
            ...
            <Grid size={{ xs: 12, md: 4 }}>
              <Card>
                <CardContent>
                  <Typography color="text.secondary" gutterBottom>Total Lots</Typography>
            ...
            <Grid size={{ xs: 12, md: 4 }}>
              <Card>
                <CardContent>
                  <Typography color="text.secondary" gutterBottom>Realized Gain/Loss</Typography>
                  <Typography
                    variant="h4"
              ...
              <Card>
                <CardContent>
                  <Typography color="text.secondary" gutterBottom>
                    ...
                    {Object.keys(currentPrices).length > 0 && (
                      <Chip label="Live" size="small" color="success" sx={{ ml: 1, height: 18, fontSize: '0.65rem' }} />
                    )}
                  </Typography>
                  <Typography
                    variant="h4"
          ...
          {holdingsArray.length > 0 && (
            <HoldingsOverview
              ...
              aiExportMenuAnchor={aiExportMenuAnchor}
              onAiExportMenuOpen={(e) => setAiExportMenuAnchor(e.currentTarget)}
              onAiExportMenuClose={() => setAiExportMenuAnchor(null)}
      ...
      {/* Price Update Dialog */}
      <Dialog open={priceDialogOpen} onClose={closePriceDialog}>
      ...
      {/* Stock Split Dialog */}
      <Dialog open={splitDialogOpen} onClose={closeSplitDialog} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            ...
            <TextField
              ...
              onChange={(e) => setNewSplit({ ...newSplit, ratio: parseFloat(e.target.value) || 1 })}
              fullWidth
              inputProps={{ step: 0.1, min: 0.1 }}
              helperText="e.g., 3 for 3:1 split (3 shares for every 1)"
            />
            <Alert severity="info">
              <Typography variant="body2">
                <strong>How it works:</strong> For a 3:1 split, quantities are multiplied by 3 and
                prices are divided by 3.
              </Typography>
            </Alert>
          </Box>
        </DialogContent>
        <DialogActions>
          <Button onClick={closeSplitDialog}>Cancel</Button>
          <Button
...
const VerificationSection: React.FC<{
  verification: any;
  realizedGainLoss: any;
}> = ({ verification, realizedGainLoss }) => (
  <Card sx={{ mb: 4 }}>
    <CardContent>
      <Typography variant="h6" gutterBottom>Verification & Debug Info</Typography>
      <Grid container spacing={2}>
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <Typography variant="body2" color="text.secondary">
            Total Sell Proceeds: {verification.totalSellProceeds.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Total Cost Basis: {verification.totalCostBasis.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </Typography>
          {realizedGainLoss.totalCommission > 0 && (
            <Typography variant="body2" color="text.secondary">
              Total Commission: {realizedGainLoss.totalCommission.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </Typography>
          )}
        </Grid>
      </Grid>
        ...
        <Box sx={{ mt: 2 }}>
          ...
          <TableContainer>
            <Table size="small">
              ...
              <TableBody>
                {verification.unmatchedSells.slice(0, 10).map((order: any, idx: number) => (
                  <TableRow key={idx}>
...
// Holdings Overview Component
const HoldingsOverview: React.FC<{
  holdingsArray: SecurityHolding[];
  recommendations: any;
  aiExportMenuAnchor: HTMLElement | null;
  onAiExportMenuOpen: (event: React.MouseEvent<HTMLElement>) => void;
  onAiExportMenuClose: () => void;
  ...
  <Card sx={{ mb: 4 }}>
    <CardContent>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2, flexWrap: 'wrap', gap: 2 }}>
        ...
        <Button variant="outlined" startIcon={<AIIcon />} onClick={onAiExportMenuOpen} color="secondary">
          Export for AI
        </Button>
        <Menu anchorEl={aiExportMenuAnchor} open={Boolean(aiExportMenuAnchor)} onClose={onAiExportMenuClose}>
          <MenuItem onClick={onExportAIActionRanges}>
            <JsonIcon sx={{ mr: 1 }} fontSize="small" />
            Request AI: Generate Action Price Ranges CSV
          </MenuItem>
          <Divider />
          <MenuItem onClick={onExportAIMetadata}>
            <JsonIcon sx={{ mr: 1 }} fontSize="small" />
          ...
          <TableBody>
            {holdingsArray.map((holding) => (
              <TableRow key={holding.security}>
                <TableCell>
                  <Typography variant="body2" fontWeight="bold">{holding.security}</Typography>
                </TableCell>
                <TableCell align="right">{holding.totalQuantity}</TableCell>
                <TableCell align="right">{holding.averageBuyPrice.toFixed(2)}</TableCell>
                <TableCell align="right">
                  {holding.totalCost.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </TableCell>
                <TableCell align="right">
                  {holding.currentPrice ? holding.currentPrice.toFixed(2) : <Chip label="Not Set" size="small" color="warning" />}
                </TableCell>
                <TableCell align="right">
                  {holding.marketValue ? holding.marketValue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '-'}
                </TableCell>
                <TableCell align="right">
                  {holding.unrealizedGainLoss !== undefined ? (
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 0.5, color: holding.unrealizedGainLoss >= 0 ? 'success.main' : 'error.main' }}>
                      {holding.unrealizedGainLoss >= 0 ? <TrendingUpIcon fontSize="small" /> : <TrendingDownIcon fontSize="small" />}
                      {holding.unrealizedGainLoss >= 0 ? '+' : ''}
                      {holding.unrealizedGainLoss.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </Box>
                  ) : '-'}
                </TableCell>
                <TableCell align="right">
                  {holding.unrealizedGainLossPercent !== undefined ? (
                    <Chip
                      ...
                      size="small"
                      color={holding.unrealizedGainLossPercent >= 0 ? 'success' : 'error'}
                    />
                  ) : '-'}
                </TableCell>
                ...
                <TableCell align="center">
                  <Button size="small" variant="outlined" onClick={() => onSetPrice(holding.security, holding.currentPrice)}>
                    Set Price
                  </Button>
                </TableCell>
              </TableRow>
  ...
  return (
    <Card>
      <CardContent>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2, flexWrap: 'wrap', gap: 2 }}>
          <Box>
            <Typography variant="h6" gutterBottom>Lot-wise Gain/Loss Analysis</Typography>
          ...
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', justifyContent: 'flex-end', flex: 1 }}>
            <TextField
              ...
              sx={{ minWidth: 200 }}
            />
            <Button variant="contained" startIcon={<DownloadIcon />} onClick={onExportMenuOpen} color="primary">
              Export
            </Button>
            <Menu anchorEl={exportMenuAnchor} open={Boolean(exportMenuAnchor)} onClose={onExportMenuClose}>
          ...
          holdingsArray.map((holding) => {
            ...
            return (
              <Accordion key={holding.security} defaultExpanded sx={{ mb: 2 }}>
                <AccordionSummary expandIcon={<ExpandMoreIcon />} sx={{ backgroundColor: 'action.hover', '&:hover': { backgroundColor: 'action.selected' } }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', pr: 2 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      <Typography variant="h6" sx={{ color: 'primary.main' }}>{holding.security}</Typography>
                      <Chip label={`${holding.totalQuantity.toFixed(0)} shares`} size="small" variant="outlined" />
                    ...
                    <Button size="small" variant="outlined" startIcon={<AddIcon />} onClick={(e) => { e.stopPropagation(); onAddSplit(holding.security); }}>
                      Add Split
                    </Button>
                  </Box>
                </AccordionSummary>
                <AccordionDetails>
                  {securitySplits.length > 0 && (
                    <Box sx={{ mb: 2 }}>
                      <Typography variant="subtitle2" sx={{ mb: 1, color: 'text.secondary' }}>Stock Splits Applied:</Typography>
                          ...
                          <TableBody>
                            {securitySplits.map((split) => (
                              <TableRow key={split.id}>
                                ...
                                <TableCell align="right">
                                  <Chip label={`${split.ratio}:1`} size="small" color="primary" variant="outlined" />
                                </TableCell>
                                <TableCell align="center">
                                  <Button size="small" color="error" startIcon={<DeleteIcon />} onClick={() => onRemoveSplit(split.id)}>
                                    Remove
                                  </Button>
                                </TableCell>
                              </TableRow>
                    ...
                    <Table size="small" stickyHeader>
                      ...
                      <TableBody>
                        {holding.lots.map((lot) => {
                          ...
                          const unrealizedGainLoss = currentPrice && lot.remainingQuantity > 0 ? lot.remainingQuantity * (currentPrice - lot.buyPrice) : undefined;
                            ...
                            <TableRow key={lot.id}>
                              <TableCell>
                                ...
                                {lot.splitRatio && lot.splitRatio !== 1 && (
                                  <Chip label={`${lot.splitRatio.toFixed(2)}x split`} size="small" color="info" sx={{ ml: 1 }} />
                                )}
                              </TableCell>
                              <TableCell align="right">
                                <Box>
                              ...
                              <TableCell align="right">
                                {lot.remainingQuantity > 0 ? (
                                  <Chip label={lot.remainingQuantity} size="small" color="primary" variant="outlined" />
                                ) : (
                                ...
                                {lot.sellOrders.length > 0 ? (
                                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5, alignItems: 'flex-end' }}>
                                    {lot.sellOrders.map((sell, idx) => (
                                      <Box key={idx} sx={{ display: 'flex', alignItems: 'center', gap: 0.5, fontSize: '0.75rem' }}>
                                  ...
                                  <Typography variant="body2" color="text.secondary" sx={{ fontStyle: 'italic' }}>Not sold</Typography>
                                )}
                              </TableCell>
                              <TableCell align="right">
                                {totalRealizedGainLoss !== 0 ? (
                                  <Chip label={`${totalRealizedGainLoss >= 0 ? '+' : ''}${totalRealizedGainLoss.toFixed(2)}`} size="small" color={totalRealizedGainLoss >= 0 ? 'success' : 'error'} />
                                ) : '-'}
                              </TableCell>
                              <TableCell align="right">
                                {unrealizedGainLoss !== undefined ? (
                                  <Chip label={`${unrealizedGainLoss >= 0 ? '+' : ''}${unrealizedGainLoss.toFixed(2)}`} size="small" color={unrealizedGainLoss >= 0 ? 'success' : 'error'} variant="outlined" />
                                ) : '-'}
                              </TableCell>
                            </TableRow>
                          );
